/**
 * Client-side life-event helpers (birthday / anniversary).
 * Mirrors worker/lib/lifeEvents.ts — keep the two in sync.
 */
...
export function dismissKey(event: LifeEventCandidate): string {
  return `fv:life-event-dismiss:${event.memberId}:${event.kind}:${event.occurrenceYear}`;
}
