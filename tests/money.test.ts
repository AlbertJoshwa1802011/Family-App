...
import { computeBalances, settledByDestination } from "../worker/lib/money";
import {
  createTestEnv,
  seedActor,
  seedFamily,
...
describe("money balance helpers", () => {
...
describe("money destinations API", () => {
  it("create → list destinations; duplicate active name → 409", async () => {
    const create = await req("POST", "/api/money/destinations", member.cookie, {
      familyId,
      name: "Mom",
      kind: "person",
    });
    ...
    const dup = await req("POST", "/api/money/destinations", owner.cookie, {
      familyId,
      name: "mom",
    });
    expect(dup.status).toBe(409);

    const church = await req("POST", "/api/money/destinations", owner.cookie, {
      familyId,
      name: "Church",
      kind: "organization",
    });
    ...
    const list = await req(
      "GET",
      `/api/money/destinations?familyId=${familyId}`,
      owner.cookie,
    );
  ...
  it("archives used destinations on DELETE; hard-deletes unused", async () => {
    ...
    const archive = await req(
      "DELETE",
      `/api/money/destinations/${mom.destination.id}`,
      owner.cookie,
    );
    ...
    const hard = await req(
      "DELETE",
      `/api/money/destinations/${unused.destination.id}`,
      owner.cookie,
    );
    ...
    const withArchived = await (
      await req(
        "GET",
        `/api/money/destinations?familyId=${familyId}&includeArchived=1`,
        owner.cookie,
      )
  ...
  it("rejects outsiders and missing familyId", async () => {
    const noFamily = await req("GET", "/api/money/destinations", owner.cookie);
    ...
    const patchDenied = await req(
      "PATCH",
      `/api/money/destinations/${created.destination.id}`,
      outsider.cookie,
      { name: "Nope" },
    );
  ...
  it("Zod rejects empty destination name", async () => {
    const res = await req("POST", "/api/money/destinations", owner.cookie, {
      familyId,
      name: "  ",
    });
...
describe("money movements + summary API", () => {
  ...
  it("received + settled updates available / in-hand / per-destination totals", async () => {
    ...
    const received = await req("POST", "/api/money/movements", member.cookie, {
      familyId,
      type: "received",
      amount: 50000,
    ...
    const toMom = await req("POST", "/api/money/movements", member.cookie, {
      familyId,
      type: "settled",
      amount: 15000,
      destinationId: momId,
    ...
    const toChurch = await req("POST", "/api/money/movements", owner.cookie, {
      familyId,
      type: "settled",
      amount: 5000,
    ...
    const summary = await req(
      "GET",
      `/api/money/summary?familyId=${familyId}`,
      owner.cookie,
    );
  ...
  it("settled without destinationId → 400; cross-family destination → 400", async () => {
    ...
    const missing = await req("POST", "/api/money/movements", owner.cookie, {
      familyId,
      type: "settled",
      amount: 100,
    });
    ...
    const otherDest = await (
      await req("POST", "/api/money/destinations", outsider.cookie, {
        familyId: otherFamilyId,
        name: "Other Mom",
      })
    ).json() as { destination: { id: string } };

    const cross = await req("POST", "/api/money/movements", owner.cookie, {
      familyId,
      type: "settled",
      amount: 100,
    ...
    const ok = await req("POST", "/api/money/movements", owner.cookie, {
      familyId,
      type: "settled",
      amount: 50,
      destinationId: momId,
  ...
  it("member cannot edit/delete owner's movement; author can", async () => {
    ...
    const forbidden = await req(
      "PATCH",
      `/api/money/movements/${created.movement.id}`,
      member.cookie,
      { amount: 1 },
    );
  ...
  it("rejects invalid amount and unknown deep path", async () => {
    const bad = await req("POST", "/api/money/movements", owner.cookie, {
      familyId,
      type: "received",
      amount: -5,
    });
    ...
    const deep = await req(
      "GET",
      `/api/money/movements/nope/extra`,
      owner.cookie,
    );
  ...
  it("filters movements by destinationId", async () => {
    ...
    const filtered = await req(
      "GET",
      `/api/money/movements?familyId=${familyId}&destinationId=${momId}`,
      owner.cookie,
    );
