/** Shared money/settlement helpers — destinations, ledger math, display. */

import {
  MONEY_MOVEMENT_TYPES,
  SETTLEMENT_DESTINATION_KINDS,
} from "../db/schema";
import { fromCents, formatMoney, toCents } from "./expenses";
...
const TYPE_SET = new Set<string>(MONEY_MOVEMENT_TYPES);
const KIND_SET = new Set<string>(SETTLEMENT_DESTINATION_KINDS);

export function isMoneyMovementType(value: string): value is MoneyMovementType {
  return TYPE_SET.has(value);
}

export function isSettlementDestinationKind(
  value: string,
): value is SettlementDestinationKind {
...
export function settledByDestination(
  movements: { type: string; amountCents: number; destinationId: string | null }[],
): Map<string, number> {
