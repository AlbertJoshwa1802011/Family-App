...
import { requireAppRole } from "../middleware/requireAppRole";
...
accessRoutes.post("/demo-requests", zv(demoRequestSchema), async (c) => {
  ...
  // Soft-dedupe: if a pending request already exists for this email, don't spam.
  ...
  // Already approved → tell them to sign in.
  ...
  if (notifyTo) {
    await sendEmail(c.env, {
      to: notifyTo,
      subject: `Demo request: ${data.name.trim()} <${email}>`,
  ...
  await sendEmail(c.env, {
    to: email,
    subject: "We received your Family Vault demo request",
...
// GET /access/admin/demo-requests?status=pending
accessRoutes.get("/admin/demo-requests", requireSession, async (c) => {
  const gate = await requireAppRole(c, "super_admin");
  if (gate instanceof Response) return gate;

  const status = c.req.query("status") ?? "pending";
  if (!["pending", "approved", "rejected", "all"].includes(status)) {
    return c.json({ error: "validation_error", issues: [{ message: "invalid status" }] }, 400);
  }
  ...
  const rows =
    status === "all"
      ...
      : await db
          .select()
          .from(schema.demoRequests)
          .where(eq(schema.demoRequests.status, status as "pending" | "approved" | "rejected"))
          .orderBy(desc(schema.demoRequests.createdAt), desc(sql`"demo_requests".rowid`));
...
// POST /access/admin/demo-requests/:id/approve
accessRoutes.post(
...
// POST /access/admin/demo-requests/:id/reject
accessRoutes.post(
...
// GET /access/admin/grants
accessRoutes.get("/admin/grants", requireSession, async (c) => {
  const gate = await requireAppRole(c, "super_admin");
  if (gate instanceof Response) return gate;
...
// POST /access/admin/grants — invite someone to the app without a demo form.
accessRoutes.post("/admin/grants", requireSession, zv(grantSchema), async (c) => {
  const gate = await requireAppRole(c, "super_admin");
  if (gate instanceof Response) return gate;
...
// POST /access/admin/grants/revoke
accessRoutes.post("/admin/grants/revoke", requireSession, zv(revokeSchema), async (c) => {
  const gate = await requireAppRole(c, "super_admin");
  if (gate instanceof Response) return gate;
