...
export const settlementRoutes = new Hono<HonoEnv>();
...
settlementRoutes.get("/summary", requireSession, async (c) => {
...
settlementRoutes.get("/destinations", requireSession, async (c) => {
  const familyId = c.req.query("familyId");
  if (!familyId) return c.json({ error: "familyId query param required" }, 400);
...
settlementRoutes.delete("/destinations/:id", requireSession, async (c) => {
  const { id } = c.req.param();
  const userId = c.get("userId")!;
  const db = getDb(c.env);
...
settlementRoutes.get("/movements", requireSession, async (c) => {
  const familyId = c.req.query("familyId");
  if (!familyId) return c.json({ error: "familyId query param required" }, 400);
...
settlementRoutes.get("/movements/:id", requireSession, async (c) => {
  const { id } = c.req.param();
  const db = getDb(c.env);
...
settlementRoutes.delete("/movements/:id", requireSession, async (c) => {
  const { id } = c.req.param();
  const userId = c.get("userId")!;
  const db = getDb(c.env);
